...
import net.lingala.zip4j.io.FileEntry;
import net.lingala.zip4j.io.IZipEntry;
...
//				}
	...
	public static boolean checkExists(String path) throws ZipException {
		if (!isStringNotNullAndNotEmpty(path)) {
			throw new ZipException("path is null");
		}
		
		return checkExists(new FileEntry(new File(path)));
	}
	
	public static boolean checkExists(IZipEntry entry) throws ZipException {
		if (entry == null) {
			throw new ZipException("cannot check if entry exists: input is null");
		}
		return entry.exists();
	}
	...
	public static long getLastModifiedFileTime(IZipEntry entry, TimeZone timeZone) throws ZipException {
		if (entry == null) {
			throw new ZipException("input is null, cannot read last modified file time");
		}
		
		if (!entry.exists()) {
			throw new ZipException("input does not exist, cannot read last modified file time");
		}
	...
	public static long getLength(String file) throws ZipException {
		if (!isStringNotNullAndNotEmpty(file)) {
			throw new ZipException("invalid file name");
		}
		
		return getLength(new FileEntry(new File(file)));
	}
	
	public static long getLength(IZipEntry entry) throws ZipException {
		if (entry == null) {
			throw new ZipException("input is null, cannot calculate length");
		}
