...
public class CipherOutputStream extends BaseOutputStream {
	...
	public void putNextEntry(IZipEntry entry, ZipParameters zipParameters) throws ZipException {
		if (zipParameters == null) {
			throw new ZipException("zipParameters is null");
		}

		if (!zipParameters.isSourceExternalStream() && entry == null) {
			throw new ZipException("input file is null");
		}
		
		if (!zipParameters.isSourceExternalStream() && !Zip4jUtil.checkExists(entry)) {
	...
	private void createFileHeader() throws ZipException {
		...
		if (zipParameters.isSourceExternalStream()) {
			...
			fileHeader.setLastModFileTime((int) Zip4jUtil.javaToDosTime((Zip4jUtil.getLastModifiedFileTime(sourceEntry, zipParameters.getTimeZone()))));
			fileHeader.setUncompressedSize(sourceEntry.length());
			fileName = Zip4jUtil.getRelativeFileName(sourceEntry.getPath(), zipParameters.getRootFolderInZip(), zipParameters.getDefaultFolderPath());
		}
		...
		int fileAttrs = 0;
		if (!zipParameters.isSourceExternalStream())
			fileAttrs = sourceEntry.getAttributes();
		...
		if (zipParameters.isSourceExternalStream()) {
			...
			fileHeader.setDirectory(this.sourceEntry.isDirectory());
		}
		if (fileHeader.isDirectory()) {
			...
			if (!zipParameters.isSourceExternalStream()) {
				long fileSize = Zip4jUtil.getLength(sourceEntry);
	...
	public void setSourceEntry(IZipEntry sourceEntry) {
		this.sourceEntry = sourceEntry;
	}

	public IZipEntry getSourceEntry() {
		return sourceEntry;
	}
}
